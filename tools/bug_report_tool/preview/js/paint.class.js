...
export default class Paint {
    ...
    drawShape() {
        ...
        if (this.tool == "line") {
            ...
            this.context.moveTo(this.startPos.x, this.startPos.y)
            this.context.lineTo(this.currentPos.x, this.currentPos.y)
            if(this.currentPos.x>this.startPos.x && this.currentPos.y>this.startPos.y){
                this.context.lineTo(this.currentPos.x-30-30, this.currentPos.y-30)
                this.context.lineTo(this.currentPos.x-30, this.currentPos.y-30-30)
            }else if(this.currentPos.x>this.startPos.x && this.currentPos.y<this.startPos.y){
                this.context.lineTo(this.currentPos.x-30, this.currentPos.y+30)
                this.context.lineTo(this.currentPos.x+30, this.currentPos.y-30)
            }else if(this.currentPos.x<this.startPos.x && this.currentPos.y>this.startPos.y){
                this.context.lineTo(this.currentPos.x+30, this.currentPos.y-30)
                this.context.lineTo(this.currentPos.x-30, this.currentPos.y+30)
